...
\item |\coloremojisetup{|\Meta{設定}|}|\Means
  パラメタ設定（\ref{sec:Parameters}）を変更する。
  \Meta{設定}は\Meta{キー}|=|\Meta{値}|,…|の形のリスト。
  以降も同様。
...
規定する名前が利用できる。
その他に以下に定める\Strong{独自短縮名}が利用できる。
\Note これらはemoji sequenceの入力の便宜のためのものである。

\newcommand*{\cTC}{\multicolumn{2}{l}}
\begin{quote}\small\begin{tabular}{@{\quad}llll@{\quad}}
\hline
|+|         & U+200D  & & (ZWJ)\\
...
\begin{itemize}
\item 絵文字を構成するUnicode文字(※)の符号値の16進表記
  （0埋め無し、大文字）を順に|-|でつないだものを「符号値列」とする。
  \Note ただしEVS（U+FE0F）は除外される。
  例えば、
  \CE{2️⃣} 〈0023 FE0F 20E3〉%
  に対する「符号値列」は|23-20E3|となる。

\item \EG{\Meta{prefixの値}\Meta{符号値列}|.|\Meta{extensionの値}}%
  がパス名である。
\end{itemize}
